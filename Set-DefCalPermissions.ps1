...
# 
# Adapted by BenschiBox 07-02-2022
# https://github.com/BenschiBox/M365-Scripts
# 
# This Script utilizes the Exchange Online PowerShell V2 commands (Get-EXO...)
...
foreach ($User in $Users) {
    ...
    if ($Exception -Contains ($User.UserPrincipalName)) {
        $ExceptionUPN = $User.UserPrincipalName
        Write-Host "$ExceptionUPN is an exception, don't touch permissions" -ForegroundColor Red
    }
    else {
